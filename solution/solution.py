...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    def cut(value, width=40):
        text = " ".join(str(value).split())
        return text if len(text) <= width else text[: width - 3] + "..."

    headers = ["Prompt", "GPT-4o Response", "Mini Response",
               "GPT-4o Latency", "Mini Latency"]
    rows = [
        [
            cut(r.get("prompt", "")),
            cut(r.get("gpt4o_response", "")),
            cut(r.get("mini_response", "")),
            f"{r.get('gpt4o_latency', 0.0):.2f}s",
            f"{r.get('mini_latency', 0.0):.2f}s",
        ]
        for r in results
    ]
...
if __name__ == "__main__":
    ...
    stats = run_assistant(
        persona="Bạn là trợ giảng thân thiện của khóa AI thực hành. "
                "Luôn trả lời bằng tiếng Việt, ngắn gọn trong 3–5 câu, dùng ví dụ "
                "đời thường cho người mới bắt đầu. Nếu không chắc chắn thì nói rõ "
                "là không chắc, không bịa thông tin. Khi người học hỏi bài tập, "
                "gợi ý hướng làm thay vì đưa đáp án hoàn chỉnh.",
    )
    print("\n--- Thống kê phiên chat ---")
